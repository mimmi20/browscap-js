...
suite('checking for issue 1971. (1 test)', function () {
  test('issue-1971-A ["AppEngine-Google; (+http://code.google.com/appengine; appid: s~snapchat-proxy)"]', function () {
    const browscap = new Browscap();
    const browser = browscap.getBrowser('AppEngine-Google; (+http://code.google.com/appengine; appid: s~snapchat-proxy)');

    assert.strictEqual(browser['Comment'], 'Snapchat Proxy', 'Expected actual "Comment" to be \'Snapchat Proxy\' (was \'' + browser['Comment'] + '\'; used pattern: ' + browser['browser_name_regex'] + ')');
    assert.strictEqual(browser['Browser'], 'Snapchat Proxy', 'Expected actual "Browser" to be \'Snapchat Proxy\' (was \'' + browser['Browser'] + '\'; used pattern: ' + browser['browser_name_regex'] + ')');
